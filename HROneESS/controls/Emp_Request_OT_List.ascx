<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Emp_Request_OT_List.ascx.cs" Inherits="Emp_Request_OT_List" %>
...
<input type="hidden" id="EmpID" runat="server" name="ID" />
    <table width="100%" border="0" cellpadding="5" cellspacing="0" class="pm_list_section">
        <tr >
            <td colspan="8" class="pm_list_title">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        <td colspan="2">
                            <asp:Label ID="Label1" runat="server" Text="CL Requisition" /> :
                        </td>
                    </tr>
                    <tr>
                        <td >
...
--%>
        ...
        ShowHeader="false" OnItemCommand="Repeater_ItemCommand" OnItemDataBound="Repeater_ItemDataBound">
            <ItemTemplate> 
                <tr >
                    <td class="pm_list">
                        <input type="hidden" runat="server" id="_RequestOTClaimID" />       
                        <a href="ESS_EmpRequestDetail.aspx?TargetEmpID=<%#binding.getValue(Container.DataItem,"EmpID")%>&EmpRequestRecordID=<%#binding.getValue(Container.DataItem,"RequestOTClaimID")%>&EmpRequestID=<%#binding.getValue(Container.DataItem,"EmpRequestID")%>">
                        <%#binding.getFValue(Container.DataItem, "RequestOTClaimCreateDate", "yyyy-MM-dd HH:mm:ss")%>                                </a>
                        
                    </td>
                    <td class="pm_list">
                        <%#binding.getFValue(Container.DataItem, "RequestOTClaimPeriodFrom", "yyyy-MM-dd")%>
                    </td>
                    <td class="pm_list">
                        <%#binding.getFValue(Container.DataItem, "RequestOTClaimPeriodTo", "yyyy-MM-dd")%>
                    </td>
                    <td class="pm_list">
                        <%#binding.getFValue(Container.DataItem, "RequestOTClaimHourFrom", "HH:mm")%>
                    </td>
                    <td class="pm_list">
                        <%#binding.getFValue(Container.DataItem, "RequestOTClaimHourTo", "HH:mm")%>
                    </td>
                    <td class="pm_list">
