...
{
    /// <summary>
    /// Summary description for BOCIPMPFFile
    /// </summary>
    public class BOCIPMPFFile : GenericMPFFile
    {
        ...
        public BOCIPMPFFile(DatabaseConnection dbConn)
            : base(dbConn)
        {
        }
        ...
        public override FileInfo GenerateMPFFile()
        {
            ...
            foreach (MPFPeriod mpfPeriod in MPFPeriodList)
            {
                ...
                periodRecordHeader[0] = mpfPeriod.PeriodFrom.ToString("MM-dd-yyyy");
                periodRecordHeader[1] = mpfPeriod.PeriodTo.ToString("MM-dd-yyyy");
                periodRecordHeader[2] = "0";
                periodRecordHeader[3] = mpfPeriod.DetailStringList.Count.ToString();
                periodRecordHeader[4] = "0";
                periodRecordHeader[5] = "0";
                periodRecordHeader[6] = string.Empty;
                periodRecordHeader[7] = string.Empty;
                periodRecordHeader[8] = string.Empty;
                string periodRecordHeaderString = string.Join(FIELD_DELIMITER, periodRecordHeader);

                mpfFileData += periodRecordHeaderString + RECORD_DELIMITER;
                foreach(string mpfContributionString in mpfPeriod.DetailStringList)
                    mpfFileData += mpfContributionString + RECORD_DELIMITER;
            }


            //HashCount = 1;

            string[] mpfFileHeader = new string[12];
            mpfFileHeader[0] = "Pay Easy EE+Cont";
            mpfFileHeader[1] = "2.3d";
            mpfFileHeader[2] = "BOCIP";
            mpfFileHeader[3] = MPFPlanSchemeNo;
            mpfFileHeader[4] = m_SequenceNo.ToString();
            mpfFileHeader[5] = "0";
            mpfFileHeader[6] = AppUtils.ServerDateTime().ToString("MM-dd-yyyy");
            mpfFileHeader[7] = "0";
            mpfFileHeader[8] = MPFPeriodList.Count.ToString();    //No. of Period
            ...
            if (PaymentMethod == PaymentMethodEnum.CHEQUE)
                ...
                mpfFileHeader[9] = "CASH";
            else
                mpfFileHeader[9] = string.Empty;
            mpfFileHeader[10] = m_ChequeNum;  //  Cheque No
            mpfFileHeader[11] = ((double)TotalAdditionalEmployeeMC
            ...
            string encryptFilePath=HROne.Lib.Entities.ESystemParameter.getParameter(dbConn, HROne.Lib.Entities.ESystemParameter.PARAM_CODE_MPF_FILE_BOCI_ENCRYPT_PATH);
            if (System.IO.File.Exists(encryptFilePath))
            {
                FileInfo resultEncrypt = GenerateTempFileName();
                resultEncrypt.Delete();
                System.Diagnostics.Process process = new System.Diagnostics.Process();
                process.StartInfo.FileName = encryptFilePath;
                process.StartInfo.Arguments = "\"" + result.FullName + "\"" + " \"" + resultEncrypt.FullName + "\"";
                ...
                if (resultEncrypt.Exists)
                {
                    m_FileIsEncrypted = true;
                    result.Delete();
        ...
        {
            return MPFPlanSchemeNo + "BOCIP" + m_SequenceNo.ToString("0000") + (m_FileIsEncrypted ? string.Empty : "-PlainText") + MPFFileExtension();
        }

        public override string MPFFileExtension()
        {
            if (m_FileIsEncrypted)
                return ".pae";
            else
                return ".txt";
        }
